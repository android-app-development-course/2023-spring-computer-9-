...
class all : Fragment() {
    ...
    override fun onCreateView(
        ...
        val root: View = inflater.inflate(R.layout.all, container, false)
        val recyclerView = root.findViewById<RecyclerView>(R.id.allList)
        ...
        adapter.setOnItemClickListener(object :
            ...
            override fun onClick(position: Int, v: View?) {
                ...
                val mdb: MyDatabase = allList[position]
    ...
    private fun editMenu(position: Int, v: View) {
        ...
        popupMenu.setOnMenuItemClickListener { item: MenuItem ->
            if (item.itemId == R.id.delete) {
                Thread {
                    val inOut: Boolean = allList[position].inOut
                    context?.let { DatabaseAction.getInstance(it)?.getAllIncomesDao?.delete(allList[position]) }
                    allList = context?.let {
                        DatabaseAction.getInstance(it)?.getAllIncomesDao
                            ?.getAllAccounts(MainActivity.username)
                    } as List<MyDatabase>
                    val msg = Message()
                    msg.what = COMPLETED
    ...
    companion object {
        ...
        val COMPLETED:Int = -1
        var allList:List<MyDatabase> = arrayListOf()
        var adapter: AccountItemAdapter = AccountItemAdapter(allList)
        ...
        fun newInstance(param1: String?, param2: String?): com.example.cashbook.ui.detailed_accounts.all? {
            val fragment: com.example.cashbook.ui.detailed_accounts.all = com.example.cashbook.ui.detailed_accounts.all()
            val args = Bundle()
            args.putString(com.example.cashbook.ui.detailed_accounts.all.ARG_PARAM1, param1)
            args.putString(com.example.cashbook.ui.detailed_accounts.all.ARG_PARAM2, param2)
